...
import { Menu, X, Shield, LayoutDashboard, Database, ExternalLink, Sparkles, MapPin } from 'lucide-react';
      ...
      <div className="bg-[#050505] text-zinc-300 text-[11px] font-mono-tech py-1.5 px-3 sm:px-6 border-b border-zinc-800 flex items-center justify-between overflow-x-auto whitespace-nowrap">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            title={isSupabaseConfigured ? 'Supabase Database Connected' : 'Configure Supabase Database'}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded bg-zinc-900 border border-white/20 text-zinc-200 hover:text-white text-[10px] transition-colors"
          >
            <Database className={`w-3 h-3 ${isSupabaseConfigured ? 'text-emerald-400' : 'text-zinc-400'}`} />
            <span>{isSupabaseConfigured ? 'DB: Live' : 'Supabase'}</span>
          </button>
        </div>
      </div>
        ...
        <div className="h-18 max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-2.5">
            ...
            <button
              ...
              className="btn-neon-primary px-4 sm:px-5 py-2.5 rounded-full text-xs font-bold text-black tracking-wider flex items-center gap-1.5 uppercase shadow-md"
            >
